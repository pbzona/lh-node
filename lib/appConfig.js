...
class Configuration {
  ...
  loadDependencies(pathToDeps) {
    try {
      delete require.cache[require.resolve(pathToDeps)];
      this.dependencies = require(pathToDeps);
    } catch (err) {
      console.error(`ERROR: Unable to load export from module ${pathToDeps}: ${err}`);
    }
  }
